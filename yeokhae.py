...
Z_RE = r'\p{Hani}|{{' + unicodedata.normalize('NFD', '이체자') + '\|.*?}}'
...
def main(args):
	...
	def format_pages(data):
		...
		for ka, kb in data:
			...
			for volume_and_page_number in data[(ka, kb)]:
				...
				url = f'https://ko.wikisource.org/wiki/Page:譯語類解_古_3912-5-v.1-2_GR32256_00_000{volume}.pdf/{page_number}'
	...
	for path in args.paths:
		with open(path, mode = 'r', encoding = 'utf-8') as file:
			...
			text = re.sub(r'{{reconstruct\|([^|{}]+)}}', r'\1', text)
			text = re.sub(r'{{SIC\|([^|{}]+)}}', r'\1', text) # book 2, page 85, 非 /ㅸ/

			text = re.sub(r'{{suspect\|([^|{}]+)\|([^|{}]+)}}', r'\1', text)
			text = re.sub(r'{{suspect\|([^|{}]+)}}', r'\1', text)
